...
def gated_residual_network(
    ...
    hidden = linear_layer(hidden_layer_size, activation=None, use_time_distributed=use_time_distributed)(
        hidden
    )
...
class TemporalFusionTransformer:
    ...
    def _batch_sampled_data(self, data, max_samples):
        ...
        input_cols = [
            tup[0] for tup in self.column_definition if tup[2] not in {InputTypes.ID, InputTypes.TIME}
        ]
    ...
    def load(self, model_folder, use_keras_loadings=False):
        ...
        if use_keras_loadings:
            ...
            utils.load(
                tf.compat.v1.keras.backend.get_session(), model_folder, cp_name=self.name, scope=self.name
            )
